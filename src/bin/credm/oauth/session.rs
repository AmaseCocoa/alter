...
impl OAuth2Session {
    ...
    pub fn create_server(&mut self) -> OAuth2TemporaryServer {
        ...
        let port = local_addr.clone().to_ip().unwrap().port();

        let redirect_uri = format!("http://127.0.0.1:{}", &port);

        OAuth2TemporaryServer {
            port: port,
    ...
    pub fn get_token(&self, code: String, server: &OAuth2TemporaryServer, pkce: &PKCESecret) -> Result<OAuth2TokenResponse, Box<dyn std::error::Error>> {
        ...
        let res = client.post(format!("{}/login/oauth/access_token", self.host))
            .headers(header)
            .json(&body) 
            .send();
